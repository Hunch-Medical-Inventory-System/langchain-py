...
class OllamaLLMInitializer():
    ...
    def answer_question_with_data(self, question):
        supplies = self.database_controller.get_data_from_table("supplies", ["id", "item"])
        inventory = self.database_controller.get_data_from_table("inventory", ["id", "supply_id", "quantity"])

        formatted_supplies = " \n".join([f"{supply['id']}: {supply['item']}" for supply in supplies])
        formatted_inventory = " \n".join(
...
def main():
    ...
    ollama = OllamaLLMInitializer(model="qwen2.5", database_controller=supabase_controller, )
